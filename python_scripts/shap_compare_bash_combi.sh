...
for optimization_algorithm in "${optimization_algorithms[@]}"; do
			...
			sed -i "s/^[[:space:]]*descriptor = .*/descriptor = (\"$descriptor\",)/" backup_config.py
		        log_file="LowerFlam_${descriptor}_${optimization_algorithm}_compare_MODELS.log"
			nohup python compare_models.py > "$log_file" 2>&1 &

			wait $!
	                echo "completed all LowerFlam runs with models for $descriptor and ${optimization_algorithm} Output saved to $log_file"
...
optimization_algorithms=("Random_Search_optimizer" "Grid_Search_optimizer")
properties=("boiling_point" "Lower_Flam" "enthalpy_of_vaporization" "flashpoint" "Viscosity_as_logVis")

for property in "${properties[@]}"; do
	        for model in "${models[@]}"; do
			               for descriptor in "${descriptors[@]}"; do 
					       sed -i "s/^[[:space:]]*property = .*/property = (\"$property\",)/" backup_config.py
